...
impl TaskManager {
    ...
    fn get_curr_task_info(&self) -> TaskInfo {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        TaskInfo {
            status: TaskStatus::Running,
            syscall_times: inner.tasks[current].task_info.syscall_times.clone(),
    ...
    fn incr_syscall_times(&self, syscall_id: usize) {
        ...
        if syscall_id >= MAX_SYSCALL_NUM {
            panic!(
                "func incr_syscall_times syscall id out of bounds: {}",
                syscall_id
            );
        }
...
pub fn get_curr_task_info() -> TaskInfo {
    TASK_MANAGER.get_curr_task_info()
}

/// Increase the syscall times of the current process
